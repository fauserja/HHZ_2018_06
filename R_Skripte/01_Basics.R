...
# IN R gibt es 5 Basis-Klassen (atomic)
class(a)  # numeric: Dezimalzahlen mit Nachkommastellen
class(5)
...
class(x)
Dienstag  # erzeugt Fehler, da Objekt `Dienstag` nicht vorhanden. 
          # Text muss mit `"` oder `'` als solcher markiert werden.
...
# Falls Elemente unterschiedliche Klassen zusammengefügt werden, werden die 
# Elemente zwangsweise umgewandelt (coersion)
vec5 <- c("Alter:", 40)
c("text", TRUE)
c(5.1, TRUE)
class(c(5, TRUE))
...
# Übersicht über wesentliche R-Funktionen
...
length(vec1)  # Länge eines Vektors = Anzahl Elemente
...
# Benamung der vorkommenden Faktorstufen `levels` durch Argument `labels`
...
geschl2[11] <- "neutral"  # Nur Werte für bestehende Levels sind erlaubt
...
# Spezialfall: `data.frame` ist Liste deren Elemente alle die selbe Länge haben
...
# Auswahl bestimmter Zeilen durch logisches Subsetting
d$alter > 15        # logischer Vektor
d[ d$alter > 15,]   # Subsetting basierend auf logischem Vektor
