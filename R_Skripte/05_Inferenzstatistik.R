...
library(ggplot2)
library(dplyr)
library(car)  # Wird für Levene-Test (Abschnitt 4) benötigt
library(tidyr)
...
# Folgende Grafik zur Erklärung von Quantilen die im qqplot visualisiert werden
dat %>% 
  ggplot(aes(x = avgTimeOnPage)) +
  stat_ecdf()

# qq-Plot zur Beurteilung der Normalverteilungsannahme
...
# Prüfung der Normalverteilung muss auf Gruppenebene erfolgen
dat %>% 
...
# Multipler Test zur Berücksichtigung der alpha-Fehler-Inflation
TukeyHSD(anova)
